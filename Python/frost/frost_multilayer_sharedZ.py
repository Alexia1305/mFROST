...
def frost_multilayer_sharedZ(X_list, r, numTrials=3, maxiter=1000, delta=1e-6, time_limit=300, init_method='USENC',
    ...
    - Z_l is a nonnegative structured orthogonal matrix encoding community memberships
  

    Notes
    -----
    The matrices Z are not stored explicitly but represented with:

    - v : ndarray of shape (n,) — share community assignments defining V,
      where v[i] is the community index of node i.

    - w : ndarray of shape (n,) — non-zero values 

    Thus, each row i of Z has a single nonzero element:
    ...
    for trial in range(numTrials):
        ...
        if init_partition is not None:
            ...
            w = initialize_w_values(sum(X_list, csr_matrix(X_list[0].shape)) / len(X_list), v)
            ...
            if init_method == 'random':
                ...
                _, v, _ = initialize_Z_onelayer(X_list[lr], r)
                w = initialize_w_values(sum(X_list, csr_matrix(X_list[0].shape)) / len(X_list), v)
...
def update_Z(X_list, degrees_layers, S, w, v):
    ...
    for i in np.random.permutation(n):
        ...
        for k in range(r):
            ...
            for l in range(L):
                c3 += S2[l, k, k]
                c1 += 2 * (wp2[l,k] - (w[i] * S[l, v[i], k]) ** 2) - 2 * S[l, k, k] * Xii[l, i]
                X = X_list[l]
                start = X.indptr[i]
                end = X.indptr[i + 1]
                ...
                c0 += -4 * np.sum(selected_vals * w[selected_cols] * S[l, v[selected_cols], k])
        ...
        for l in range(L):
            for k in range(r):
                wp2[l, k] = wp2[l, k] - (w[i] * S[l, v[i], k]) ** 2 + (wi_new * S[l, int(vi_new), k]) ** 2
...
def update_S(X_list, r, w, v):
    ...
    for l in range(L):
        ...
        for k in range(len(val)):
            S[l, v[i[k]], v[j[k]]] += w[i[k]] * w[j[k]] * val[k]
    return S
...
def initialize_Z_alllayers(X_list, r, init_method):
    ...
    for l in range(L):
        _, v_init[l], Z[l] = initialize_Z_onelayer(X_list[l], r)
    ...
    w = initialize_w_values( sum(X_list, csr_matrix(X_list[0].shape)) / len(X_list), labels_final)

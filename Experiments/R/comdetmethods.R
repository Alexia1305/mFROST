  ...
  if(method == "spherical-onenetwork") {
   ...
   #################################################################################
    ...
    if (method=="mfrost"){
      ...
      res <- frost$mfrost(X_list, K, init_method='USENC',init_seed=seed_for_python,numTrials=3L,time_limit=1000)
   ...
   # Initialized by DC_MASE
    if (method=="frost-dcmase"){
      ...
      res <- frost$mfrost(X_list, K, init_partition=init_partition,init_seed=seed_for_python,numTrials=3L,time_limit=1000)
      labels <- py_to_r(res[[2]])
      community_memberships <- as.vector(labels + 1)
    }
  ...
  if(method == "spherical-onenetwork") {
   ...
   #################################################################################
    ...
    if (method=="mf"){
      ...
      res <- frost$mfrost(X_list, K, maxiter=as.integer(0),init_method='USENC',init_seed=seed_for_python)
      labels <- py_to_r(res[[2]])
      community_memberships <- labels + 1 
      }
